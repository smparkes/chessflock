...
(function($){
  ...
  var Beginning = new ChessFlock.Class(function Beginning(ui) {
    ...
    join: function(event) {
      event.stopPropagation();
      this.ui.join(this.div.find("input").val());
      this.destroy();
    },
    content: function() {
      if (!this.div) {
        ...
        this.div.append(
          $("<div>or </div>").
            ...
            append(" a game with").
            append("<input type='text' value='"+"chess@chessflock"+"'></input>"));
  ...
  ChessFlock.UI.Single = new ChessFlock.UI.Class(function Single(){
    ...
    join: function(game) {
      (this.game = new ChessFlock.Game.Name(game+"#chessflock")).join(this.view);
    }
  });
}(jQuery));