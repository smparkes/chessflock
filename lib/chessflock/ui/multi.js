...
(function($){
  ...
  ChessFlock.UI.Multi = new ChessFlock.UI.Class(function Multi(){
    global.multi = this;
    var self = this;
    $(global.document.body).append(
      $("<button class='monitor'>monitor</button>").click(function(){
        self.monitor = global.open("about:blank","events");
      })
    );
    this.first = $(".game");
    ...
    this.resize();
       ...
       log_to_monitor: function(s) {
         if (this.monitor) {
           $(this.monitor.document.body).prepend(
             $("<div></div>").text(s)
           );
         }
       },
       ...
       new_view: function() {
         ...
         if (this.first) {
           div = this.first;
           this.first = false;
           svg_div = $(div).find(".board");
         } else {
           div = $("<div class='game'></div>").
             append( $("<div class='id'></div>") ).
             append( svg_div = $("<div class='board'></div>") ).
             append( $("<div class='opponent'></div>") ).
             appendTo(this.element);
         }
         view = new Actor(global.svg_x = new ChessFlock.Board.View.SVG(svg_div));
         global.svg_x.log_to_monitor = $.proxy(this.log_to_monitor,this);
       ...
       view_event: function(ev) {
         throw new Error("implement multi view event");
       },
       game_event: function(game, cause) {
         if (cause instanceof Dramatis.Exception.Terminated) {
           ...
           this.drop_game(Actor.id(game));
         } else {
           global.console.debug("implement game event type "+cause.constructor+" "+cause);
         }
       },
       resign: function(game, view) {
         throw new Error("implement multi resign");
       },
       ...
       join: function(opponent) {
         ...
         if (global.localStorage) {
           ...
           if (game_id) {
             global.console.debug("abandoning game "+game_id);
         ...
         if (global.localStorage) {
           global.console.debug("recording game "+game_id+" for "+opponent_route);
         ...
         parent.find(".id .text").text("Game ID: "+game_id);
