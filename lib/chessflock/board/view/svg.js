...
(function($){
  ...
  ChessFlock.Board.View.SVG =
    new ChessFlock.Board.View.Class(function SVG(element){
      ...
      page_to_offset: function(start, pageX, pageY) {
        var svg = $(this.svg);
        var svg_bb = svg[0].getBBox();
        var offset = svg.offset();
        var width = svg.width();
      ...
      move: function(start, view, piece) {
        ...
        svg.
          bind("mousemove",function(ev) {
            ...
            console.debug(position);
            var offset = self.page_to_offset(start,ev.pageX,ev.pageY);
            view.offset(offset);
          }).
      ...
      turn_on: function() {
        var self = this;
        var color = self.state.colors[self.index];
        var svg = $(self.svg);
        svg.
          bind("mousedown",function(ev){
            ...
            self.move(self.page_to_board(ev.pageX, ev.pageY), view, piece);
          });
      },
      turn_off: function() {
        $(this.svg).
