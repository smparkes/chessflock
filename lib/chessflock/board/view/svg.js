  ...
  new ChessFlock.Board.View.Class(function SVG(element){
    ...
    ChessFlock.$(element).svg({
      ...
      onLoad: function(wrapper) {
        ...
        var x = 29;
        var y = 405;
        wp.attr("transform","rotate(0 "+x+" "+y+")");
        if (true) {
          wpx.animate({
            svgTransform: t
          },{
        ...
        if (false) {
          ...
          self.$("svg > g").animate({
            ...
            duration: 750,
            complete: function() {
              self.$("svg .white .piece,svg .black .piece").attr("transform","scale(1,1)");
              self.$("svg .white .piece,svg .black .piece").animate({
                // svgTransform: "rotate(10 25 25)"
                svgTransform: "scale(1,.95)"
              },2000);
            }
          });
        }
    ...
    populateSquares: function() {
      ...
      for (var row = 0; row < 8; row++) {
        for (var column = (row === 0 ? 2 : 0); column < 8; column++) {
          ...
          copy.setAttribute("transform","translate("+delta_x+" "+delta_y+")");
