...
(function($){
  ...
  ChessFlock.Board.View.SVG =
    new ChessFlock.Board.View.Class(function SVG(element){
      ...
      if (original && ChessFlock.Board.View.SVG.inited) {
        $(self.element).append(self.svg = $(original).clone());
        after();
      } else {
        ChessFlock.Board.View.SVG.inited = true;
        self.svg = ChessFlock.Board.View.SVG.svg;
        after();
      }
    }, [
      Super,
      ...
      place: function(piece, position, animate) {
        // FIX: factor/cleanup
        // global.console.debug("place",piece.position, position, animate);
        var remove;
        var new_position;
        if ((typeof position === "number") && (piece.position !== position)) {
          new_position = position;
          animate = true;
        } else if ((position === void(0) || position === null) && animate) {
          new_position = position;
          animate = true;
          remove = true;
        } else {
        ...
        if (piece.svg) {
          piece.svg.place(new_position, animate, remove);
        }
      },
      page_to_position: function(pageX,pageY) {
        var svg = $(this.svg);
        var svg_bb = svg[0].getBBox();
        var offset = svg.parent().offset();
      ...
      move: function(start, view, piece) {
        ...
        svg.
          bind("mousemove",function(ev) {
            ...
            view.offset(offset);
            self.tell(piece.position, offset );
          }).
          bind("mouseup",function(ev){
            ...
            var position = self.page_to_position(ev.pageX,ev.pageY);
            // FIX: chess rules
            if (self.positions[position] && self.positions[position].piece.color === self.color) {
              view.place(void(0), true);
              self.tell(piece.position, void(0));
            } else {
              if (self.tell_timeout) {
                global.clearTimeout(self.tell_timeout);
                delete self.tell_timeout; 
              }
      ...
      touchmove: function(start, view, piece) {
        var self = this;
        var svg = $(self.svg);
        // global.console.debug(piece.type);
        svg.
          ...
          bind("touchend",function(ev){
            ...
            svg.unbind("touchmove").unbind("touchend");
          });
      },
      update_piece: function(position, offset) {
        // global.console.debug("update piece",position, offset);
        var view = this.positions[position];
      ...
      turn_on: function() {
        ...
        svg.
          bind("touchstart",function(ev){
            ...
            ev.stopPropagation();
            ev = ev.originalEvent;
            // ChessFlock.growl("env: "+ev.touches);
            if (!self.log_to_monitor) {
              if (self.opponent) {
                self.opponent.send("log_to_monitor",[$.print(ev)]);
                self.opponent.send("log_to_monitor",[$.print(ev.touches.length)]);
              }
            }
            ev = ev.touches[0];
            var position = self.page_to_position(ev.pageX,ev.pageY);
            // ChessFlock.growl("pos: "+position,"sticky");
            ...
            // ChessFlock.growl("pos: "+piece+" "+piece.color+" "+self.color+" "+piece.type);
            ...
            self.touchmove(start, view, piece);
          });
        svg.
          bind("mousedown",function(ev){
            if (false && !self.log_to_monitor) {
              if (self.opponent) {
                self.opponent.send("log_to_monitor",[$.print(ev)]);
              }
            }
      ...
      tell: function(position, offset) {
        var self = this;
        self.current_tell = [ position, offset ];
        if (!self.tell_timeout) {
          self.last_tell = self.current_tell;
          self.notify(self.last_tell[0], self.last_tell[1]);
          self.tell_timeout = global.setTimeout(function(){
            if (!self.compare_tell(self.current_tell, self.last_tell)) {
              self.last_tell = self.current_tell;
              self.notify(self.last_tell[0], self.last_tell[1]);
              self.tell_timeout = global.setTimeout(arguments.callee,100);
            } else {
              delete self.tell_timeout;
            }
          }, 100);
  ...
  ChessFlock.Board.View.SVG.Piece =
    ...
    place: function(new_position, animate, remove) {
      if ((new_position === void(0) || new_position === null) && animate && remove) {
        $(this.svg).animate({opacity: 0}, "normal");
        return;
      }
