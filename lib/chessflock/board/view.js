...
(function(){
  ...
  ChessFlock.Board.View = new ChessFlock.Board.Class(function View(){
    ...
    attach: function(game, Continue) {
      var self = this;
      global.console.debug('attach '+Actor.id(this),this.constructor+"");
      global.console.debug('attach game',$.print(game));
      this.game = game;
      console.debug("mr",Actor.id(this))
      this.game.state(c({
        value: [self, function(state) {
          global.console.debug("state resp");
          this.state = state;
...
try{
  ...
  global.console.debug("tr",diff.turn,typeof diff.turn,$.print(this.turn));
