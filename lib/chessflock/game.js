...
ChessFlock.Game = new Dramatis.Actor.Type(new ChessFlock.Class(function Game(){
  ...
  rejoin: function(view) {
    var view_route = Dramatis.Actor.Name.route(view)+"";
    for(var i=0; i<this._players.length; i++) {
      var player_route = Dramatis.Actor.Name.route(this._players[i])+"";
  ...
  destroy: function() {
    ...
    if (global.localStorage) {
      var id = Actor.Interface.id(this);
      global.localStorage.removeItem(id);
      Actor.Interface.terminate(this);
    }
  },
  _state: function() {
    return ({
